...
class Room(arcade.View):
    global globalVars
    def __init__(self,window,tilemap,x,y,scale,menu,questions,questionsMenu,previousRoom = None, nextRoom = None):
        ...
        self.questionMenu = questionsMenu
        
        for i in self.questionMenu:
            i.gameView = self
        ...
        sounds.walk.stop()
        self.passList = []
        for count,i in enumerate(self.scene.get_sprite_list("Key")):
            i.questionMenu = self.questionMenu[count]
            self.passList.append(i.questionMenu.canPass)
    ...
    def on_key_press(self, key: int, modifiers: int):
        ...
        if key == arcade.key.E and arcade.check_for_collision_with_list(self.player,self.scene["Key"]):
            for i in arcade.check_for_collision_with_list(self.player,self.scene["Key"]):
                self.window.show_view(i.questionMenu)
    ...
    def on_update(self, delta_time: float):
        ...
        if False in self.passList:
            self.canPass = False
        else:
            self.canPass = True
        
        
        print("Viendo los question menus")
        for i in self.scene.get_sprite_list("Key"):
            print(i.questionMenu.canPass)
            if i.questionMenu.canPass:
                i.texture = self.fillHeart
            else:
                i.texture = self.emptyHeart
            self.passList = []
            self.passList.append(i.questionMenu.canPass)
